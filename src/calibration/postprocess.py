...
def reshape_token_logits_by_beam(
    scores: Tuple[Tensor], batch_size: int, num_beams: int
):
    token_logits: Tensor = torch.stack(scores, dim=1)
    _, sequence_length, vocab_size = token_logits.shape
...
def decode_beam_search_sentences(
    ...
    return list_of_sentences, sequences
